# -*- coding: utf-8 -*-
# --- START OF FILE airport_scheduling_grasp.py ---
...
import time
import copy
import matplotlib.pyplot as plt # Solo si mantienes los gráficos

# --- Type Aliases ---
PlaneIdx = int
PistIdx = int

# Formatos de la solución del Greedy Estocástico (proveniente de tu "Item 1")
InitialSchedule = Dict[PlaneIdx, int]
InitialRunwayAssignment = Dict[PlaneIdx, PistIdx]

# Formato interno para GRASP, HC y evaluación principal
RunwaySequence = List[PlaneIdx]
RunwayLayout = Dict[PistIdx, RunwaySequence] # {pista_id: [avion_id1, avion_id2, ...]}
FinalSchedule = Dict[PlaneIdx, int] # {avion_id: tiempo_aterrizaje_final}


# --- FUNCIONES PROVENIENTES DEL "ITEM 1" (O SIMILARES) ---
def read(path: str) -> Tuple[int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    try:
        ...
        line_idx += 1
        E_list, P_list, L_list, Ci_list, Ck_list, tau_rows = [], [], [], [], [], []
        ...
        for i in range(num_planes):
            ...
            E_list.append(int(parts[0])); P_list.append(int(parts[1])); L_list.append(int(parts[2]))
            Ci_list.append(float(parts[3])); Ck_list.append(float(parts[4]))
            ...
            if len(current_sep_row) != num_planes: 
                raise ValueError(f"Sep. avión {i}: {num_planes} esperados, {len(current_sep_row)} obtenidos.")
            
            tau_rows.append(current_sep_row)
        if len(E_list) != num_planes: 
            raise ValueError("Cuenta de datos de avión incorrecta.")
        
        E = np.array(E_list); P_val = np.array(P_list); L = np.array(L_list)
        Ci = np.array(Ci_list); Ck = np.array(Ck_list); tau = np.array(tau_rows)
...
def calc_cost(schedule: InitialSchedule, P_val: np.ndarray, Ci: np.ndarray, Ck: np.ndarray) -> float:
    """Calcula el costo total de penalización para un horario dado (formato InitialSchedule)."""
    ...
    P_scheduled = P_val[plane_indices]
    Ci_scheduled = Ci[plane_indices]
    Ck_scheduled = Ck[plane_indices]
    antes = np.sum(Ci_scheduled * np.maximum(0, P_scheduled - landing_times))
    tarde = np.sum(Ck_scheduled * np.maximum(0, landing_times - P_scheduled))
    cost = antes + tarde
...
) -> Tuple[Optional[InitialSchedule], Optional[InitialRunwayAssignment], float, bool]:
    

    if seed is not None: random.seed(seed)
    unscheduled_planes = set(range(num_planes))
    schedule: InitialSchedule = {}
    runway_assignment: InitialRunwayAssignment = {}
        ...
        for k_plane in unscheduled_planes:
            for runway_idx in range(num_runways):
                ...
                if last_plane_idx[runway_idx] != -1:
                    ...
                    separation_needed = tau[last_plane_idx[runway_idx]][k_plane]

                    if separation_needed >= 99999: 
                        continue


                    earliest_after_separation = last_landing_time[runway_idx] + separation_needed

                earliest_possible_time = max(min_start_time, earliest_after_separation)

                if earliest_possible_time <= L[k_plane]: 
                    candidates.append((k_plane, runway_idx, earliest_possible_time))
...
def build_deterministic_greedy_solution( 
    num_planes: int, E: np.ndarray, P_val: np.ndarray, L: np.ndarray,
    Ci: np.ndarray, Ck: np.ndarray, tau: np.ndarray,
    num_runways: int = 1
) -> Tuple[Optional[InitialSchedule], Optional[InitialRunwayAssignment], float, bool]:
    ...
    for k in plane_indices:
        ...
        for runway_idx in range(num_runways):
            ...
            if actual_landing_time <= L[k] and actual_landing_time < best_landing_time:
                best_landing_time = actual_landing_time; best_runway = runway_idx
...
def convert_initial_to_runway_layout(
    initial_schedule: Optional[InitialSchedule], initial_assignment: Optional[InitialRunwayAssignment],
    num_planes: int, num_pist: int
) -> Optional[RunwayLayout]:
    ...
    layout: RunwayLayout = {p: [] for p in range(num_pist)}
    details = []
...
def get_final_schedule_from_layout(
    runway_layout: RunwayLayout, num_pist: int, E: np.ndarray, L: np.ndarray, tau: np.ndarray
...
def evaluate_runway_layout(
    runway_layout: RunwayLayout, num_pist: int, E: np.ndarray, P_val: np.ndarray, L: np.ndarray,
    Ci: np.ndarray, Ck: np.ndarray, tau: np.ndarray
) -> Tuple[float, Optional[FinalSchedule], bool]:
    

    final_schedule, feasible = get_final_schedule_from_layout(runway_layout, num_pist, E, L, tau)
...
def local_search_hill_climbing(
    initial_layout: RunwayLayout, num_pist: int, E: np.ndarray, P_val: np.ndarray, L: np.ndarray,
    Ci: np.ndarray, Ck: np.ndarray, tau: np.ndarray, use_best_improvement: bool
) -> Tuple[RunwayLayout, float]:
    

    current_layout = copy.deepcopy(initial_layout)
    current_cost, _, feasible = evaluate_runway_layout(current_layout, num_pist, E, P_val, L, Ci, Ck, tau)
        ...
        for r_idx in range(num_pist): # Intra-Runway Swaps
            ...
            for i in range(len(seq)):
                ...
                for j in range(i + 1, len(seq)):
                    ...
                    cost_n, _, feas_n = evaluate_runway_layout(n_layout, num_pist, E, P_val, L, Ci, Ck, tau)
...
def build_grasp_construction_solution(
    ...
    layout: RunwayLayout = {r: [] for r in range(num_pist)}
...
def grasp_algorithm(
...
) -> Tuple[Optional[RunwayLayout], float, List[float]]:
    best_overall_layout: Optional[RunwayLayout] = None
    ...
    for i_ws in range(num_stochastic_warm_starts):
        ...
        best_layout_this_ws_run: Optional[RunwayLayout] = None
        ...
        for hc_restart_attempt in range(max_restarts_in_warm_start_hc + 1):
            ...
            base_layout = convert_initial_to_runway_layout(initial_sched, initial_assign, num_planes, num_pist)
            if base_layout is None: continue
            
            cost_base_eval,_,feas_base_eval=evaluate_runway_layout(base_layout,num_pist,E,P,L,Ci,Ck,tau)
            if not feas_base_eval: continue

            hc_layout, hc_cost = local_search_hill_climbing(base_layout, num_pist,E,P,L,Ci,Ck,tau,use_best_improvement_hc)
    ...
    for iter_g in range(max_grasp_iterations):
        ...
        constructed_layout = build_grasp_construction_solution(num_planes,num_pist,E,P,L,Ci,Ck,tau,alpha_rcl_grasp)
        ...
        hc_layout, hc_cost = local_search_hill_climbing(constructed_layout,num_pist,E,P,L,Ci,Ck,tau,use_best_improvement_hc)
    ...
    final_c,_,final_f = evaluate_runway_layout(best_overall_layout,num_pist,E,P,L,Ci,Ck,tau)
...
def print_runway_solution(
    ...
    runway_layout: Optional[RunwayLayout] = None, num_pist: Optional[int] = None):
...
def main():
    ...
    try:
        ...
        if layout_fi and cost_fi!=float('inf'): 
            _,sched_fi,feas_fi = evaluate_runway_layout(layout_fi,num_pists,E,P_array,L,Ci,Ck,tau)
        ...
        if layout_bi and cost_bi != float('inf'): 
            _,sched_bi,feas_bi = evaluate_runway_layout(layout_bi,num_pists,E,P_array,L,Ci,Ck,tau)
...
if __name__ == '__main__':
    main()
# --- END OF FILE airport_scheduling_grasp.py ---